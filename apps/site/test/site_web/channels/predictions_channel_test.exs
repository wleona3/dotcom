defmodule SiteWeb.PredictionsChannelTest do
  ...
  setup do
    reassign_env(:site, :predictions_subscribe_fn, fn route_id, _ ->
      case route_id do
        @route_39 ->
          [@prediction39]
  ...
  describe "handle_info/2" do
    test "pushes new data onto the socket", %{socket: socket} do
      ...
      {:ok, _, socket} =
        subscribe_and_join(socket, PredictionsChannel, "predictions:#{@route_39}@#{@stop_fh}")
