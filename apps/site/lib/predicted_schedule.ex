defmodule PredictedSchedule do
  ...
  * prediction: The prediction for this trip (optional)
  ...
  @doc """
  Returns the vehicle crowding for a given PredictedSchedule.
  """
  @spec vehicle_crowding(PredictedSchedule.t()) :: Vehicles.Vehicle.crowding() | nil
  def vehicle_crowding(predicted_schedule) do
    with %Trip{id: trip_id} <- trip(predicted_schedule),
         %Vehicles.Vehicle{crowding: crowding} <- Vehicles.Repo.trip(trip_id) do
      crowding
    else
      _ ->
        nil
  ...
  @spec headsign(__MODULE__.t()) :: Trip.headsign() | nil
  def headsign(predicted_schedule) do
  ...
  @doc """
  Builds a %PredictedSchedule{} struct from a map containing a schedule and a prediction
  """
  @spec from_maps(map) :: __MODULE__.t()
  def from_maps(%{schedule: schedule, prediction: prediction}) do
    %PredictedSchedule{
      schedule: maybe_struct_from_map(Schedule, schedule),
      prediction: maybe_struct_from_map(Prediction, prediction)
    }
  end

  def from_maps(%{schedule: schedule}) do
    %PredictedSchedule{
      schedule: maybe_struct_from_map(Schedule, schedule),
      prediction: nil
    }
  ...
  def from_maps(%{prediction: prediction}) do
    %PredictedSchedule{
      schedule: nil,
      prediction: maybe_struct_from_map(Prediction, prediction)
    }
  ...
  @spec maybe_struct_from_map(module | struct, map | nil) :: struct
  defp maybe_struct_from_map(module, %{} = map) do
    struct(module, Map.to_list(map))
  end
  ...
  @spec headsign_data(__MODULE__.t()) :: headsign_data()
