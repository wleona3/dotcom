defmodule SiteWeb.PlacesController do
  ...
  def autocomplete(conn, %{"input" => input, "hit_limit" => hit_limit_str, "token" => token}) do
    autocomplete_fn =
      Map.get(conn.assigns, :autocomplete_fn, &LocationService.Autocomplete.autocomplete/2)
  ...
  def details(conn, %{"place_id" => place_id}) do
    geocode_fn =
      Map.get(conn.assigns, :geocode_by_place_id_fn, &LocationService.Geocode.geocode/1)
  ...
  def reverse_geocode(conn, params) do
    reverse_geocode_fn =
      Map.get(
        conn.assigns,
        :reverse_geocode_fn,
        &LocationService.ReverseGeocode.reverse_geocode/2
      )
