defmodule SiteWeb.PredictionsChannel do
  ...
  def join("predictions:" <> route_id_and_stop_id, _message, socket) do
    predictions_subscribe_fn =
      Application.get_env(
        :site,
        :predictions_subscribe_fn,
        &PredictionsPubSub.subscribe/2
      )

    [route_id, stop_id] = String.split(route_id_and_stop_id, "@")
    predictions = predictions_subscribe_fn.(route_id, stop_id)
