defmodule Predictions.PredictionsPubSubTest do
  ...
  describe "subscribe/2" do
    ...
    test "clients get existing predictions upon subscribing", %{pid: pid} do
      ...
      :sys.replace_state(pid, fn state ->
        Map.put(state, :predictions_by_key, %{"#{@route_39}@#{@stop_id}" => predictions})
      end)

      assert PredictionsPubSub.subscribe(@route_39, @stop_id, pid) == predictions
    end
  end
  ...
  describe "handle_info/2 - {:reset, predictions}" do
    setup do
      ...
      :sys.replace_state(pid, fn state ->
        Map.put(state, :predictions_by_key, %{"#{@route_39}@#{@stop_id}" => [1, 2, 3]})
      end)
    ...
    test "broadcasts new predictions lists to subscribers", %{pid: pid} do
      PredictionsPubSub.subscribe(@route_39, @stop_id, pid)
  ...
  describe "handle_info/2 - {:update, predictions}" do
    setup do
      ...
      :sys.replace_state(pid, fn state ->
        Map.put(state, :predictions_by_key, %{"#{@route_39}@#{@stop_id}" => [@prediction39]})
      end)
